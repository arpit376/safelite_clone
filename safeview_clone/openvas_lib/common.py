...
def get_connector(host, username, password, port=9390, timeout=None):
    ...
    if manager.protocol_version in ("4.0", "5.0", "6.0", "7.0"):
        ...
        raise RemoteVersionError("Unknown OpenVAS version for remote host: {}".format(manager.protocol_version))
...
class ConnectionManager(object):
    ...
    def make_xml_request(self, xmldata, xml_result=False):
        ...
        if etree.tostring(response) == etree.tostring(etree.ElementTree()):
